"""Клиент для работы с ЛОКАЛЬНОЙ Ollama (на вашем компьютере)."""
import requests
...
class OllamaLocalModelConfig:
    """Конфигурация моделей для локальной Ollama."""
...
class OllamaLocalClient:
    ...
    def check_connection(self) -> bool:
        """Проверяет доступность локальной Ollama."""
        try:
    ...
    def _call_model(self, model: str, prompt: str, timeout: tuple = None) -> str:
        ...
        try:
            ...
            raise TimeoutError(f"Превышено время ожидания от модели {model}")
        except Exception as e:
    ...
    def review_annotation(self, text: str, initial_annotation: str) -> tuple:
        ...
        # Парсим ответ (формат: ===СТАТУС===, ===ОТЧЁТ===, ===ПЕРЕСКАЗ===)
        annotation = initial_annotation
        status = "passed"
